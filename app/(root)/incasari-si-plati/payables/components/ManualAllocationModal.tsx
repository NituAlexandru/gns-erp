...
import { createManualSupplierAllocation } from '@/lib/db/modules/financial/treasury/payables/supplier-allocation.actions'
import { useEffect, useState } from 'react'

interface ManualAllocationModalProps {
  invoice: UnpaidSupplierInvoice | null
  payment: PopulatedSupplierPayment | null
  onClose: () => void
  onAllocationCreated: () => void 
}

// Schema Zod DOAR pentru acest formular
...
export function ManualAllocationModal({
  ...
  async function onSubmit(data: AllocationFormValues) {
    ...
    // Validări suplimentare de siguranță
    if (amountToAllocate > round2(payment.unallocatedAmount)) {
      toast.error('Suma depășește suma nealocată a plății.')
      return
    }
    ...
    try {
      const result = await createManualSupplierAllocation({
  ...
  return (
     <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            Alocă Manual Factura {invoice.invoiceNumber}
          </DialogTitle>
          <DialogDescription asChild>
           <div className='space-y-3'>
              <p className='text-muted-foreground'>
                Introdu suma din plată pe care vrei să o aloci acestei facturi.
              </p>

              <div className='p-3 border rounded-md bg-muted'>
                <div className='text-sm space-y-1 text-muted-foreground'>
                  <div className='flex justify-between'>
                    <span>Plată Nealocată:</span>
