        ...
        >
          {image ? (
            <Link className='flex items-end' href={`/admin/products/${id}`}>
              <Image
                className='rounded border  aspect-square object-scale-down max-w-full h-auto  mr-1'
                src={image!}
                alt={label}
                width={36}
                height={36}
              />
              <p className='text-left text-sm whitespace-nowrap overflow-hidden text-ellipsis'>
                {label}
              </p>
            </Link>
          ) : (
